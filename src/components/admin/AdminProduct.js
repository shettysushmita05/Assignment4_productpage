...
import callDelete from "./Portals/Portal.js"
...
const AdminProduct =({checkupdate,setCheckUpdate})=>{
    ...
    const API =async()=>{
        try{
            ...
            // apply the filter if it is there 

            if (filter.length > 0){
                data=data.filter(product => product.category.name === filter[0]&&product.price<=filter[1]);
            }
            if(searchQuery.length > 0){
                data=data.filter(product => product.title.toLowerCase().includes(searchQuery.toLowerCase())
            );
        }

        // set the total count based on the filtered data
            settotal(data.length);
            // set pagination based on the total count
    ...
    useEffect(()=>{
        API();
    },[currentpage,filter,searchQuery,checkupdate]);
    ...
    return(
        ...
        <div className="flex gap-4 items-center">
            ...
            <div className="w-screen max-w-screen-md leading-6">
                <form className="relative mx-auto flex justify-between w-full max-w-2xl items-center rounded-md border shadow-lg">
    ...
    <button type="submit" class="absolute right-0 mr-1 font-bold inline-flex h-full items-center justify-center rounded-lg bg-blue-700 px-10  text-white  " disabled>Search</button>
        ...
        {portal && <Portal Portal={Portal} onclose={onclose} isPortal={isPortal} id={getid} setCheckUpdate={setCheckUpdate}/>}
      {Data.length<=0&&<div className="flex justify-center"><img src={NoItemLandingPage} alt="" /></div>}
          ...
          <tbody >
            {Data.map((item, index) => (
              <tr key={index} className="p-2 shadow-lg rounded-md m-3 flex items-center justify-between">
                ...
                <td><button className="font-bold w-6 h-6 text-xlflex items-center justify-center" onClick={() => callDelete(item.id,setCheckUpdate)}>
  <img src={cancleImage} alt=""/>
</button>
<p>Right Arrow: &#8594;</p>
</td>
              </tr>
